...
class ApprovalExtraTest(ApprovalTestConfiguration):
    def test_request_approval(self):
        ...
        with self.assertRaises(exceptions.UserError):
            self.request_one(self.obj_2, self.user_1)
    ...
    def test_archive(self):
        views = self.approval_type_2.get_primary_views()
        existed_view = self.env["ir.ui.view"]
        for view in views:
            existed_view |= self.approval_type_2.get_extended_view(view)
        self.assertEqual(len(views), 2)
        self.assertEqual(len(existed_view), 2)
        self.approval_type_2.action_archive()
        self.assertEqual(len(existed_view.exists()), 2)
        ...
        self.assertFalse(existed_view.exists())
        domain0 = self.ApprovalType.with_user(self.user_2.id).domain_get(
            self.model_name
        )
    ...
    def test_change_approver(self):
        ...
        change_form = Form(
            self.env["change.approver"]
            .with_context(
                active_model=request1._name,
