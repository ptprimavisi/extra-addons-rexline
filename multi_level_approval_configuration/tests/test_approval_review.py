...
class ApprovalReviewTest(ApprovalTestConfiguration):
    def test_configure(self):
        ...
        self.assertEqual(
            sorted(str(expect_dm)), sorted(str(domain0)), "Domain is not correct"
        )
        ...
        # Check priority
        types = (
            self.env["multi.approval.type"]
            .with_user(self.user_1.id)
            ._get_types(self.model_name)
        )
        ...
        for f_name in f_names:
            self.assertTrue(f_name in mail_fields, f"Field {f_name} does not exists")
