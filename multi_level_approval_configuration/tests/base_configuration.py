...
class ApprovalTestConfiguration(TransactionCase):
    ...
    def setUpClass(cls):
        ...
        cls.approval_type_4 = cls.new_approval_type(
            cls, 4, priority=10, defaults={"group_ids": [(6, 0, cls.user_group.ids)]}
        )
    ...
    def new_user(self, no):
        ...
        user.partner_id.email = f"dmuser_{no}@test.com"
        return user

    def new_employee(self, no, user):
        e = self.env["hr.employee"].create(
            {"name": f"Demo Employee {no}", "user_id": user.id}
        )
        return e

    def new_approval_type(self, no, state="to_approve", priority=100, defaults=None):
        if defaults is None:
            defaults = {}
        vals = {
            "name": f"Approval Type {no}",
            "model_id": self.model_name,
            "apply_for_model": True,
            "domain": f"[('state', '=', '{state}')]",
            "priority": priority,
            "line_ids": [
                (
                    0,
    ...
    def new_object(self, no, user, state="draft"):
        return self.Obj.with_user(user.id).create(
            {"name": f"New object {no}", "state": state}
        )
